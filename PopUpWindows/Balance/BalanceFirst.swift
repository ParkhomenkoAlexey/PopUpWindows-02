...
class BalanceStartAlertView: UIView {
    ...
    init(image: UIImage, userName: String, balanceAccaunt: Int ) {
        super.init(frame: UIScreen.main.bounds)
        self.imageView.image = image
        //self.specialtyLabel.text = specialtyLabel
        self.userName = userName
        self.balancedLabel.text = "(\(balanceAccaunt) руб.)"
    ...
    @objc func actionButtonPressed() {
        SwiftEntryKit.transform(to: BalanceEnterAlertView(image: #imageLiteral(resourceName: "pocket"), userName: "Илья"))
    }
}
...
extension BalanceStartAlertView {
    func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            balancedLabel.topAnchor.constraint(equalTo: specialtyLabel.bottomAnchor, constant: 23),
